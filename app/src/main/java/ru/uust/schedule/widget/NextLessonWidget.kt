 ...
 *
 * Показывает идущую пару, иначе ближайшую. Когда на сегодня пары кончились,
 * ищет первую пару следующего учебного дня — иначе вечером виджет
 * превращался бы в пустую плашку.
 */
class NextLessonWidget : GlanceAppWidget() {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        // Цель ищем до provideContent: внутри composable suspend-вызовов быть не должно.
    ...
    private suspend fun resolveTarget(context: Context, s: WidgetSnapshot): Target? {
        ...
        val todayDay = if (s.date == s.today) s.day else repo.cachedDay(s.groupId, s.today)
        ...
        repeat(7) {
            val first = repo.cachedDay(s.groupId, date)?.realLessons?.firstOrNull()
            if (first != null) return Target(first, false, DayLogic.title(date, s.today))
        ...
        Column(
            ...
            if (target == null) {
                Text(
                    "Пар впереди нет",
                    style = TextStyle(
                        color = p.textSecondary.glance(), fontSize = 14.sp,
            ...
            val argb = p.subjectArgb(lesson.subject, s.notes[lesson.subject]?.hue ?: -1)
            ...
            Row(GlanceModifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ColorPill(argb, widthDp = 3, heightDp = 13, cornerDp = 2)
                Spacer(GlanceModifier.width(7.dp))
                Text(
                    text = target.dayLabel?.let { "$it · $countdown" } ?: countdown,
                    style = TextStyle(
                        color = p.accent.glance(), fontSize = 11.sp,
                        fontWeight = FontWeight.Medium,
                    ),
                    maxLines = 1,
                )
            ...
            Text(
                text = lesson.subject,
                style = TextStyle(
                    color = p.textPrimary.glance(), fontSize = 15.sp,
